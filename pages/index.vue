<template>
  <div class="bg-woodsmoke-950 min-h-screen pb-20">
    <div class="bg-woodsmoke-975 p-6 flex justify-center">
      <span class="text-woodsmoke-50 font-semibold">Seja bem vindo ao To-do-List</span>
    </div>
        ...
        <div :class="['grid grid-cols-6 justify-between py-4 px-5 rounded mb-1 min-h-20',
          ...
          <div v-else class="col-span-6 sm:col-span-3 grid items-center gap-3">
            <label class="flex items-center gap-3 cursor-pointer">
              <input type="checkbox" v-model="task.checked" hidden @change="toggleStatus(task)" />
          ...
          <div class="gap-3 col-span-4 sm:col-span-1 flex items-center justify-end py-3">
            <ButtonDefault v-if="task.isEditing" @click.stop="saveTask(index)" texto="Salvar" variant="secondary"
              withIcon icon="save" size="xs" />
            <ButtonDefault @click="editTask(task, index)" variant="secondary" withIcon icon="edit" size="xs" />
            <ButtonDefault @click="deleteTask(index)" variant="danger" withIcon icon="trash" size="xs" />
          </div>
        </div>
      </div>
    </div>
...
const toggleStatus = (task) => {
  if (task.checked) {
    ...
    task.status = task.previousStatus || task.status;
    delete task.previousStatus;
  }
};

const defaultTaskCount = computed(() => {
  return tasks.filter(task => task.status === 'Padrão').length;
});

const importantTaskCount = computed(() => {
  return tasks.filter(task => task.status === 'Importante').length;
});

const inProgressTaskCount = computed(() => {
  return tasks.filter(task => task.status === 'Fazendo').length;
});

const standbyTaskCount = computed(() => {
  return tasks.filter(task => task.status === 'Parado').length;
});

const completeTaskCount = computed(() => {
  return tasks.filter(task => task.status === 'Finalizado').length;
});

const tasks = reactive([
  { text: 'Item list 01 - Unnamed - Undefined Description', status: 'Padrão', checked: false, isEditing: false },
  { text: 'Item list 02 - Unnamed - Undefined Description', status: 'Fazendo', checked: false, isEditing: false },
]);
...
function editTask(task, index) {
  if (task.isEditing) {
    ...
    newStatus.value = tasks[index].status;
  }
};
...
function saveTask(index) {
  ...
  console.log(tasks[index].text, index);
};

function createTask() {
  // Create a new task object
  const newTask = {
    text: createText.value, // Get the task text from the newText ref
    status: createStatus.value, // Get the task status from the newStatus ref
    checked: false, // Set the checked property to false (task not completed)
    isEditing: false, // Set the isEditing property to false (task not in edit mode)
  };
